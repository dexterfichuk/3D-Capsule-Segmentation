...
class Mask(layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        if type(inputs) is list:
            assert len(inputs) == 2
            input, mask = inputs
            _, hei, wid, dep, _, _ = input.get_shape()
            if self.resize_masks:
                mask = tf.image.resize_bicubic(mask, (hei.value, wid.value, dep.value))
...
class ConvCapsuleLayer(layers.Layer):
    ...
    def build(self, input_shape):
        assert len(input_shape) == 6, "The input Tensor should have shape=[None, input_height, input_width, input_depth" \
                                      " input_num_capsule, input_num_atoms]"
        self.input_height = input_shape[1]
        self.input_width = input_shape[2]
        self.input_depth = input_shape[3]
...
#           added 1 more self.scaling
    ...
    def compute_output_shape(self, input_shape):
        ...
        output_shape[3] = deconv_length(output_shape[3], self.scaling, self.kernel_size, self.padding)
